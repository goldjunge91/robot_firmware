...
// Use predefined Serial1 from Arduino framework for Pico
UartProtocolClass PowerBoardSerial(Serial1, PWR_BRD_SERIAL_BAUDRATE, PWR_BRD_SERIAL_CONFIG);
...
void SetLocalPower(SwitchStateTypeDef State_)
{
  ...
  if (State_ == Toggle)
    digitalWrite(EN_LOC_5V, !digitalRead(EN_LOC_5V));
}
...
{
  uint8_t DataToSend[256]; // Fixed size array instead of VLA
  DataToSend[0] = ByteAddr;
  memcpy(DataToSend + 1, Value, Size);
  I2cBus.beginTransmission(EEPROM_CONTROL_BYTE(EEPROM_DEV_ID, BlockAddr));
  if (I2cBus.write(DataToSend, Size + 1) == (size_t)(Size + 1))
  {
    I2cBus.endTransmission();
    return 0;
  }
