...
UartProtocolClass::UartProtocolClass(
    HardwareSerial &serial, uint32_t arg_baudrate, uint8_t arg_config)
    : _serial(serial)
{
  _serial.begin(arg_baudrate, arg_config);
...
{
  this->CleanRxBuffer();
  this->rx_buffer_size = this->_serial.readBytes(rx_buffer, RX_BUFF_CAPACITY);
          ...
          if (this->processed_frame.arg_size <= MAX_ARGS_SIZE)
          {
            ...
            if (
                HexToByte(
                    ...
                    &processed_frame.check_sum) != ConversionError)
            { // 1 byte of '<' + 2 bytes (CMD)
              // + 2 bytes (ARG_SIZE)
...
{
  _serial.write(arg_buffer, arg_size);
}

void UartProtocolClass::SendBuffer(uint8_t arg_size, String arg_buffer)
{
  _serial.print(arg_buffer.substring(0, arg_size));
}

void UartProtocolClass::CleanRxBuffer(void) { memset(this->rx_buffer, 0, RX_BUFF_CAPACITY); }

size_t UartProtocolClass::write(uint8_t c) { return _serial.write(c); }
void UartProtocolClass::flush(void) { _serial.flush(); }
int UartProtocolClass::available(void) { return _serial.available(); }
int UartProtocolClass::read(void) { return _serial.read(); }
int UartProtocolClass::peek(void) { return _serial.peek(); }
void UartProtocolClass::begin(unsigned long baudrate) { _serial.begin(baudrate); }
void UartProtocolClass::begin(unsigned long baudrate, uint16_t config) { _serial.begin(baudrate, config); }
void UartProtocolClass::end() { _serial.end(); }
UartProtocolClass::operator bool() { return bool(_serial); }
